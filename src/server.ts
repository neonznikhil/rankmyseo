...
import {
  createRankmyseoOAuthProvider,
  type RankmyseoOAuthEnv,
...
import { handleSelfHostedRankmyseoMcpRequest } from "@/server/mcp/transport";
...
const rankmyseoOAuthProvider = createRankmyseoOAuthProvider(appFetch);

// Authorize a RANKY agent connection in the Worker, before it reaches the Durable
...
// Route /agents/* to the RANKY chat DO. Auth happens here (both the WS upgrade
...
function handleFetch(
  ...
  if (isHostedAuthMode(authMode)) {
    ...
    return rankmyseoOAuthProvider.fetch(
      publicRequest,
      env as RankmyseoOAuthEnv,
      ctx,
    );
  }
...
// AuditScratchpad DO live in the rankmyseo-audit aux worker
  ...
  async scheduled(
    ...
    if (controller.cron === MCP_OAUTH_PURGE_CRON) {
      ...
      if (isHostedAuthMode(getAuthMode(env.AUTH_MODE))) {
        const result = await rankmyseoOAuthProvider.purgeExpiredData(
          env as RankmyseoOAuthEnv,
        );
