...
export function ProjectContextPage({ projectId }: { projectId: string }) {
  const contextQuery = useQuery({
    ...
    // 5-minute staleTime would show pre-RANKY-turn memory as current.
    staleTime: 0,
  });
  ...
  return (
    ...
    <div key={projectId} className="space-y-8">
      <p className="text-sm text-base-content/70">
        What RANKY, Claude Code, and any connected MCP client know about this
...
function ProseSections({
  ...
  // render straight from the query, so a write from RANKY shows up on refetch.
