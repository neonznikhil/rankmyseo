...
describe("maybeSendSelfHostHeartbeat", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("RANKMYSEO_TELEMETRY_DISABLED", "");
  ...
  it("does not send when RANKMYSEO_TELEMETRY_DISABLED is set", async () => {
    vi.stubEnv("RANKMYSEO_TELEMETRY_DISABLED", "1");
    const harness = createHarness();
  ...
  it('still sends when the disable flags are explicitly "0"/"false"', async () => {
    vi.stubEnv("RANKMYSEO_TELEMETRY_DISABLED", "0");
