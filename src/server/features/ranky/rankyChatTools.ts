...
// Shape of the MCP tool objects exported from src/server/mcp/tools/*. RANKY reuses
...
// Adapt one RANKMYSEO tool into an AI SDK tool. The shared handler receives the
...
// external MCP path.
//
// RANKY always runs inside one project (the session row), so we bind that project
...
// the session history. RANKY's get_audit_status therefore waits server-side:
...
/**
 * Builds RANKY's tool surface as an AI SDK ToolSet: the full MCP toolset plus the
 * free site-reading tools. Every tool the RANKMYSEO MCP server exposes is
...
export function buildRankyMcpTools(
  ...
  // Note: no `list_projects`. RANKY is bound to the session's project, so
  ...
  return {
    ...
    get_product_info: tool({
      description:
        "The RANKMYSEO fact sheet: what the product does, plans/pricing, credit costs, integrations, MCP setup. Call before answering questions about RANKMYSEO itself. Uses no credits.",
      inputSchema: z.object({}),
      execute: () => Promise.resolve({ factSheet: rankmyseoFactSheet }),
    }),
