...
// Bundle the repo's public-facing skills (.agents/skills) into RANKY at build
...
function parseSkill(path: string, raw: string): RankySkill | null {
  ...
  // GitHub issues for contributors, which RANKY has no surface for.
  if (frontmatter.name === "simple-issue-description") return null;
  // RANKY has no report tools, so the report-writing skill has nothing to drive.
