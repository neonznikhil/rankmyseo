...
// RANKY's read-only view of the project's shared memory. The block has no `set`
...
const PUBLIC_ORIGIN_KEY = "ranky-public-origin";
...
/**
 * Durable Object backing the RANKY in-app agent, built on Think. One DO per chat
...
export class RankyChatAgent extends Think {
  ...
  private buildModel(reasoningEffort: "max" | "low") {
    ...
    if (!apiKey) {
      throw new Error("OPENROUTER_API_KEY is required for the RANKY agent");
    }
  ...
  private buildSoulPrompt(): Promise<string> {
    return withPgClient(async () => {
      ...
      if (!ctx) {
        return "You are RANKY, the SEO agent inside RANKMYSEO. This chat session no longer exists; tell the user to start a new chat.";
      }
  ...
  async beforeTurn(turnCtx: TurnContext): Promise<TurnConfig> {
    ...
    return withPgClient(async (): Promise<TurnConfig> => {
      ...
      // Gate every turn on credits in hosted mode: RANKY is open to every plan
      ...
      if (hosted) {
        ...
        if (depleted) {
          ...
          return this.refusalTurn(
            "You're out of credits. Top up to keep using RANKY.",
          );
        }
      ...
      const authContext: ToolAuthContext = {
        ...
        // RANKY sessions belong to one project's workspace; org context is
  ...
  private recordSpend(costUsd: number, { flush = false } = {}): void {
    ...
    this.billing = this.billing
      ...
      .catch((error: unknown) => {
        console.error("[ranky] credit metering failed", error);
  ...
  async onChatResponse(result: ChatResponseResult): Promise<void> {
    ...
    if (result.status === "completed") {
      await withPgClient(() => this.session.refreshSystemPrompt()).catch(
        (error: unknown) => {
          console.error("[ranky] context refresh failed", error);
        },
      );
    }
  }
  ...
  onChatError(error: unknown, ctx?: ChatErrorContext): unknown {
    console.error("[ranky] chat turn error", ctx?.stage, error);
  ...
  override async onChatRecovery(
    ...
    console.warn("[ranky] interrupted turn not re-run", {
      name: this.name,
  ...
  protected override async continueLastTurn(
    ...
    if (args[1]?.trigger === "recovery-continue") {
      console.warn("[ranky] skipped queued recovery continuation", {
        name: this.name,
      });
