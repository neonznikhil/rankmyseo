...
export type ToolAuthContext = {
  ...
  // organizationId is the authorization boundary — self-host and RANKY, which
  ...
  // on the MCP path; absent on hand-built contexts (RANKY), which name
...
export type ToolContext = {
  ...
  // the `ranky:turn` event of the turn that made it. Absent for MCP transports.
  turnId?: string;
};

export const MCP_AUTH_CONTEXT_PROP = "rankmyseoAuth";
...
const applicationAuthContextSchema = z.object({
  ...
  // absent (self-host, RANKY) means "pinned".
...
export function createMcpToolContext(
  ...
  // (handleAuthenticatedRankmyseoMcpRequest); this only assembles identity.
