...
import {
  handleAuthenticatedRankmyseoMcpRequest,
  handleSelfHostedRankmyseoMcpRequest,
...
const selfHostedAuthMocks = vi.hoisted(() => ({
  ...
  createRankmyseoMcpServer: vi.fn(),
...
vi.mock("@/lib/auth", () => ({
  getHostedBaseUrl: () => "https://rankmyseo.test",
}));

vi.mock("@/server/mcp/server", () => ({
  createRankmyseoMcpServer: (props?: unknown) => {
    selfHostedAuthMocks.createRankmyseoMcpServer(props);
    return new McpServer({
      name: "RANKMYSEO MCP",
      title: "RANKMYSEO",
      version: "0.0.11",
      description: "SEO research tools for AI agents",
      websiteUrl: "https://rankmyseo.com",
      icons: [
        {
          src: "https://rankmyseo.com/android-chrome-512x512.png",
          mimeType: "image/png",
          sizes: ["512x512"],
        },
      ],
...
function createMcpRequest(headers?: Record<string, string>) {
  return new Request("https://rankmyseo.test/mcp", {
    method: "POST",
    headers: {
...
function hostedProps(scopes: string[] = ["mcp"]) {
  return createWorkersOAuthMcpProps({
    ...
    baseUrl: "https://rankmyseo.test",
    clientId: "client-1",
    scopes,
  });
}

describe("handleSelfHostedRankmyseoMcpRequest", () => {
  beforeEach(() => {
  ...
  it("accepts local no-auth MCP requests with the local admin context", async () => {
    const response = await handleSelfHostedRankmyseoMcpRequest(
      createMcpRequest(),
      "local_noauth",
      {},
      ctx,
    ...
    expect(selfHostedAuthMocks.createRankmyseoMcpServer).toHaveBeenCalledWith({
      [MCP_AUTH_CONTEXT_PROP]: {
        userId: "local-admin",
        userEmail: "admin@localhost",
        organizationId: "delegated-local-admin",
  ...
  it("answers OPTIONS preflight without resolving an auth context", async () => {
    const response = await handleSelfHostedRankmyseoMcpRequest(
      new Request("https://rankmyseo.test/mcp", { method: "OPTIONS" }),
      "cloudflare_access",
      {},
      ctx,
    );
    ...
    expect(selfHostedAuthMocks.createRankmyseoMcpServer).not.toHaveBeenCalled();
  });
});

describe("handleAuthenticatedRankmyseoMcpRequest", () => {
  beforeEach(() => {
  ...
  it("accepts the provider's encrypted identity and MCP scope fallback", async () => {
    ...
    const response = await handleAuthenticatedRankmyseoMcpRequest(
      createMcpRequest(),
      props,
      {},
      { ...ctx, props },
