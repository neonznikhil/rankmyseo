...
import { z } from "zod";
import type { createRankmyseoOAuthProvider } from "./oauth-provider";
...
vi.mock("@/lib/auth", () => ({
  getHostedBaseUrl: () => "https://app.rankmyseo.com",
}));
...
vi.mock("@/server/mcp/transport", () => ({
  handleAuthenticatedRankmyseoMcpRequest: vi.fn(),
}));
...
async function dispatch(
  provider: ReturnType<typeof createRankmyseoOAuthProvider>,
  request: Request,
) {
  ...
  return {
    ...
    props: {
      rankmyseoAuth: {
        ...
        baseUrl: "https://app.rankmyseo.com",
        clientId: "client-1",
...
describe("RANKMYSEO OAuth provider configuration", () => {
  beforeEach(() => {
  ...
  it("binds tokens and protected-resource metadata to the canonical MCP URL", async () => {
    ...
    const provider = createRankmyseoOAuthProvider(() => new Response("app"));

    await dispatch(provider, new Request("https://app.rankmyseo.com/health"));

    expect(mocks.options).toHaveLength(1);
    expect(mocks.options[0]?.resourceMetadata).toEqual({
      resource: "https://app.rankmyseo.com/mcp",
      scopes_supported: ["mcp"],
      resource_name: "RANKMYSEO MCP",
    });
  ...
  it("lets the provider issue Perplexity a real client secret", async () => {
    ...
    await dispatch(
      provider,
      new Request("https://app.rankmyseo.com/api/auth/oauth2/register", {
        method: "POST",
  ...
  it("includes the authorization-server issuer when consent is denied", async () => {
    ...
    const response = await invokeDefaultHandler(
      new Request("https://app.rankmyseo.com/api/oauth/consent", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Origin: "https://app.rankmyseo.com",
        },
        ...
        OAUTH_PROVIDER: {
          parseAuthRequest: () =>
            Promise.resolve({
              ...
              scope: ["mcp"],
              state: "state-1",
              issuer: "https://app.rankmyseo.com",
            }),
        },
      },
    );
  ...
  it("redirects safe authorization errors with state and issuer", async () => {
    ...
    const response = await invokeDefaultHandler(
      new Request("https://app.rankmyseo.com/api/auth/oauth2/authorize"),
      {
        OAUTH_PROVIDER: {
          parseAuthRequest: () =>
            Promise.reject(
