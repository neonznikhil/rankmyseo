...
import { createRankmyseoMcpServer } from "@/server/mcp/server";
...
async function handleLegacyJsonRequest(request: Request, props: McpProps) {
  ...
  // no RANKMYSEO tool issues them.
  const server = createRankmyseoMcpServer(props);
...
export async function handleAuthenticatedRankmyseoMcpRequest(
  request: Request,
  props: unknown,
  env: unknown,
...
export async function handleSelfHostedRankmyseoMcpRequest(
  request: Request,
