...
import type { handleAuthenticatedRankmyseoMcpRequest } from "@/server/mcp/transport";
...
const mocks = vi.hoisted(() => ({
  ...
  handleAuthenticatedRankmyseoMcpRequest:
    vi.fn<typeof handleAuthenticatedRankmyseoMcpRequest>(),
}));
...
vi.mock("@/server/mcp/transport", () => ({
  handleAuthenticatedRankmyseoMcpRequest:
    mocks.handleAuthenticatedRankmyseoMcpRequest,
}));
...
function request(headers?: HeadersInit, method = "POST") {
  return new Request("https://app.rankmyseo.com/mcp", { method, headers });
}
...
describe("handleMcpApiKeyRequest", () => {
  beforeEach(() => {
    ...
    mocks.handleAuthenticatedRankmyseoMcpRequest.mockResolvedValue(
  ...
  it("returns 403 when the user has no organization membership", async () => {
    ...
    expect(mocks.handleAuthenticatedRankmyseoMcpRequest).not.toHaveBeenCalled();
  });
  ...
  it("leaves non-RANKMYSEO bearer tokens for OAuth", async () => {
    await expect(
  ...
  it("leaves non-RANKMYSEO x-api-key values for OAuth", async () => {
    await expect(
      handleMcpApiKeyRequest(
        request({
