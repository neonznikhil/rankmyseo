...
type RankmyseoToolDefinition<Input extends ToolSchema> = {
  name: string;
  config: {
    title?: string;
    description?: string;
...
function registerRankmyseoTool<Input extends ToolSchema>(
  server: McpServer,
  tool: RankmyseoToolDefinition<Input>,
  authProps: McpProps,
) {
...
export function createRankmyseoMcpServer(authProps: McpProps) {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions:
        "RANKMYSEO research tools use credits. Proceed with normal focused research, but ask the user for confirmation before planned batches over 2,000 credits.",
    },
  );

  const register = <Input extends ToolSchema>(
    tool: RankmyseoToolDefinition<Input>,
  ) => registerRankmyseoTool(server, tool, authProps);
