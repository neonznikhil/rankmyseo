...
const BASE = "https://app.rankmyseo.com";
...
type Provider = ReturnType<typeof createRankmyseoOAuthProvider>;
...
beforeEach(async () => {
  ...
  provider = createRankmyseoOAuthProvider(() => new Response("app"));
...
async function callMcp(accessToken: string) {
  ...
  expect(response.status).toBe(200);
  return propsEchoSchema.parse(await response.json()).props.rankmyseoAuth;
}
