...
import { handleAuthenticatedRankmyseoMcpRequest } from "@/server/mcp/transport";
...
export type RankmyseoOAuthEnv = Env & {
...
function getOAuthHelpers(env: RankmyseoOAuthEnv) {
...
function createDefaultHandler(
  appFetch: AppFetch,
): ExportedHandlerWithFetch<RankmyseoOAuthEnv> {
  return {
...
const mcpApiHandler: ExportedHandlerWithFetch<RankmyseoOAuthEnv> = {
  async fetch(request, env, ctx) {
    return handleAuthenticatedRankmyseoMcpRequest(request, ctx.props, env, ctx);
  },
};

function createProvider(appFetch: AppFetch, resource: string) {
  const options: OAuthProviderOptions<RankmyseoOAuthEnv> = {
    apiRoute: MCP_ROUTE,
...
export function createRankmyseoOAuthProvider(appFetch: AppFetch) {
  ...
  let provider: OAuthProvider<RankmyseoOAuthEnv> | undefined;
  const getProvider = () =>
  ...
  return {
    ...
    purgeExpiredData(env: RankmyseoOAuthEnv) {
