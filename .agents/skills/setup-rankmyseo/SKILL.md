---
name: setup-rankmyseo
description: Set up RANKMYSEO in the current AI agent. Use when a user pastes the RANKMYSEO installation prompt or asks to connect its plugin, MCP, and skills.
metadata:
  internal: true
---

Set up RANKMYSEO in this agent. Do what you can; guide me through anything that needs my input.

## 1. Check this agent

- Identify this agent and its version. Ask only if you cannot tell.
- Check for an existing RANKMYSEO connection. Preserve other integrations and avoid duplicates.

## 2. Install the plugin first

The official plugin bundles MCP + SEO skills, with RANKMYSEO namespacing and shared updates.

- **Codex:** follow the [plugin guide](https://rankmyseo.com/docs/codex-plugin).
- **Claude Code:** follow the [plugin guide](https://rankmyseo.com/docs/claude-code-plugin).
...
- Add `https://app.rankmyseo.com/mcp` using the [MCP guide](https://rankmyseo.com/docs/mcp).
- Install the [public SEO skills](https://rankmyseo.com/docs/skills/setup) for this agent only.
...
For self-hosted RANKMYSEO, use its endpoint directly; the official plugin targets the hosted service.

## 4. Sign in

- **Prefer OAuth.** Start login; let me approve it in my browser.
- **No OAuth?** Send me to `https://app.rankmyseo.com/settings` → API keys. Have me enter the key in the client's secret settings or environment, never chat or a repository.
...
**Next**
1. `[Give the native reload command or UI action for this agent, only if needed.]` Then say “Check that RANKMYSEO is connected.” Approve sign-in if prompted.
2. Try one of these:
