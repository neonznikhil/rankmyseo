...
description: Verify the RANKMYSEO MCP server end-to-end on a local dev server — protocol-level correctness against real DataForSEO, then a headless-agent consumer probe that tests tool ergonomics (descriptions, schemas, output size, errors, async flows) without the maintainer manually driving an MCP client. Use after adding or changing MCP tools, or when asked to check that the MCP "works" or "is ergonomic".
metadata:
  internal: true
---
...
- Start `pnpm dev:agents` in the background. The server URL is branch-prefixed: `http://<branch-suffix>.rankmyseo.localhost:1355` (the exact URL is printed on boot; logs tee to `.logs/dev-server.log`).
...
{
  "mcpServers": {
    "rankmyseo-local": { "type": "http", "url": "http://<url>/mcp" }
  }
}
```
...
unclear errors, credit-cost surprises? Did async/taskId flows behave as described? List anything that made you hesitate or retry." \
  ...
  --allowedTools "mcp__rankmyseo-local,mcp__rankmyseo-local__*" \
