...
- [ ] `2026-09-11T00:13:05Z` — `codex` — The web-content review skill points to the removed `src/server/features/onboarding/rankmyseo-fact-sheet.md`; the reference now lives at `src/server/features/ranky/rankmyseo-fact-sheet.md`. Update the skill's pointer so content reviews reach the current fact sheet.
