...
// Preview hostnames are `rankmyseo-<stage>.<WORKERS_SUBDOMAIN>` — the naming
...
// - Stage "hosted-prod": names the EXISTING rankmyseo.com production resources
  ...
  .object({
    ...
    workflows: z.array(
      z.object({
        ...
        name: z.string(),
        class_name: z.string(),
        // Set when the workflow class lives in the rankmyseo-audit aux
...
const makeResources = (stage: string) => {
  ...
  return {
    DB: Cloudflare.D1.Database("DB", {
      name: prod ? PROD_NAMES.d1 : `rankmyseo-db-${stage}`,
    ...
    R2: Cloudflare.R2.Bucket("R2", {
      name: prod ? PROD_NAMES.r2 : `rankmyseo-r2-${stage}`,
    ...
    KV: Cloudflare.KV.Namespace("KV", {
      title: prod ? PROD_NAMES.kv : `rankmyseo-kv-${stage}`,
    }).pipe(keep),
    OAUTH_KV: Cloudflare.KV.Namespace("OAUTH_KV", {
      title: prod ? PROD_NAMES.oauthKv : `rankmyseo-oauth-kv-${stage}`,
    }).pipe(keep),
  };
};
  ...
  Effect.gen(function* () {
    ...
    if (!policyAud) {
      ...
      const application = yield* emailAccessGate({
        ...
        policyName: `rankmyseo ${stage} self-host users`,
        applicationName: `rankmyseo ${stage}`,
...
const dataEnv = {
  ...
  RANKMYSEO_TELEMETRY_DISABLED: optionalVar("RANKMYSEO_TELEMETRY_DISABLED"),
};
...
export default Alchemy.Stack(
  ...
  Effect.gen(function* () {
    ...
    if (prod) {
      ...
      if (!authUrl) {
        return yield* Effect.die(
          new Error(
            "Set BETTER_AUTH_URL (https://app.rankmyseo.com) in .env.production.",
          ),
        );
      }
    ...
    const auditWorker = yield* Cloudflare.Worker("rankmyseo-audit", {
      name: `${workerName(stage)}-audit`,
      main: "./dist/rankmyseo_audit/index.js",
      bundle: false,
      url: false,
      compatibility: {
    ...
    const app = yield* Cloudflare.Worker("rankmyseo", {
      name: workerName(stage),
      // Prod serves the real domains; the zone is inferred from the hostname.
      domain: prod ? ["app.rankmyseo.com", "www.app.rankmyseo.com"] : undefined,
      ...
      // Site audits moved to the rankmyseo-audit worker, but RankCheckWorkflow
      ...
      env: {
        ...
        // privately in the rankmyseo-audit worker). Alchemy backs new DO
      ...
      // Prod adopts the live worker serving app.rankmyseo.com; never delete it
