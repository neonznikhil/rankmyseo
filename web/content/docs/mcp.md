---
title: "Set up RANKMYSEO MCP"
description: "Connect RANKMYSEO MCP to Claude, Codex, and other AI clients."
---

RANKMYSEO MCP lets compatible AI clients call RANKMYSEO tools for keyword research, SERP inspection, local business research, competitive search intelligence, domain research, backlink overview, saved keywords, rank tracking, shared project context, and Google Search Console performance and URL inspection.
...
The first connection sends you through RANKMYSEO login. After authorization, your MCP client can call RANKMYSEO tools with the project context and account scopes you approved. For headless environments and CI, [connect with an API key](#connect-with-an-api-key) instead.

For the most current setup UI and a copyable endpoint, open [Agent setup in RANKMYSEO](https://app.rankmyseo.com/ai).

For setup prompts, plugin commands, and skill updates, see [Agent setup](/docs/agent-setup).

## Claude Code

The [RANKMYSEO plugin](/docs/claude-code-plugin) is the preferred way to connect Claude Code — one install adds MCP and the public SEO skills together. Use the steps below only if you want MCP on its own.

Use user scope to make RANKMYSEO available across projects. Use local scope for the current repository.

```bash
claude mcp add --transport http --scope user rankmyseo https://app.rankmyseo.com/mcp
```

After adding the server, approve the RANKMYSEO login when prompted.

## Claude Desktop

1. Open Customize -> Connectors.
2. Click Add (or +), then choose Add custom connector.
3. Paste `https://app.rankmyseo.com/mcp`.
4. Approve the RANKMYSEO login when prompted.
...
```json
{
  "mcpServers": {
    "rankmyseo": {
      "url": "https://app.rankmyseo.com/mcp"
    }
  }
}
```
...
The [RANKMYSEO plugin](/docs/codex-plugin) is the preferred way to connect Codex CLI — one install adds MCP and the public SEO skills together. Use the steps below only if you want MCP on its own.
...
In the [RANKMYSEO app](https://app.rankmyseo.com/settings), open **Settings -> API keys**, create a key, and copy it when it appears. It won't be shown again.

For Claude Code, run:

```bash
claude mcp add --transport http --scope user rankmyseo https://app.rankmyseo.com/mcp --header "Authorization: Bearer oseo_YOUR_KEY"
```
...
```bash
export RANKMYSEO_API_KEY=oseo_YOUR_KEY
codex mcp add rankmyseo --url https://app.rankmyseo.com/mcp --bearer-token-env-var RANKMYSEO_API_KEY
```
...
RANKMYSEO MCP exposes tools for SEO research workflows:
...
- List saved keywords from an RANKMYSEO project.
- Save useful keywords back to RANKMYSEO.
...
Once RANKMYSEO MCP is connected, [set up RANKMYSEO Agent Skills](/docs/skills/setup). MCP gives your agent access to RANKMYSEO data. Skills are separate `SKILL.md` files that tell your agent how to use that data for specific SEO jobs.
...
If your client cannot connect, check that the server URL is exactly `https://app.rankmyseo.com/mcp`.

If Codex reports `Authorization server response missing required issuer: expected https://app.rankmyseo.com`, upgrade Codex CLI or the Codex desktop app to 0.147.0 or later. Codex 0.143 through 0.146 drop the issuer from the OAuth callback. You can also [connect with an API key](#connect-with-an-api-key) instead of OAuth.

If authorization fails, disconnect the RANKMYSEO server in your client, add it again, and repeat the login flow.

If your agent cannot find a project, ask it to list RANKMYSEO projects first and use the returned project ID in later tool calls.
