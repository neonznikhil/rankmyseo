...
description: "Deploy RANKMYSEO to your own Cloudflare account for internet-facing, multi-device, or team use."
---

Host RANKMYSEO on Cloudflare for internet-facing self-hosting across multiple devices or with your team. One deploy command provisions everything, including the Cloudflare Access login gate. Works on Cloudflare's free plan.
...
## 1) Clone your RANKMYSEO repo

Fork `rankmyseo/rankmyseo` on GitHub if you want a repo you control, then clone it locally:

```bash
git clone https://github.com/YOUR_GITHUB_USER/rankmyseo.git
cd rankmyseo
corepack enable
pnpm install
```
...
3. RANKMYSEO should load after login.

If login fails, re-check `ACCESS_ALLOWED_EMAILS` and redeploy.

## Connect the MCP server through Cloudflare Access

Use the same Cloudflare Access application that protects your RANKMYSEO Worker. Managed OAuth is required for MCP clients and is not enabled by default.

1. Open Cloudflare Zero Trust.
2. Go to `Access controls` -> `Applications`.
3. Find your RANKMYSEO application, then select `Edit`.
...
## Give teammates access to RANKMYSEO

Add the teammate to `ACCESS_ALLOWED_EMAILS` in `.env.selfhost` and redeploy. Everyone allowed through shares one RANKMYSEO workspace.

## Updating to the latest RANKMYSEO version
...
- [Operations](https://github.com/rankmyseo/rankmyseo/blob/main/docs/SELF_HOSTING_CLOUDFLARE_OPERATIONS.md): telemetry and other day-to-day tasks.
- [Legacy deployments](https://github.com/rankmyseo/rankmyseo/blob/main/docs/SELF_HOSTING_CLOUDFLARE_LEGACY.md): maintenance for installs created with the retired Deploy-button or manual Wrangler flows.
