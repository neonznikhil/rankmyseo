...
![A quote card reading: Every morning boot the computer up, open up Claude, and it goes using MCP connectors, goes into all of my accounts, scrapes everything. So at a top level I can see everything that needs to be done for clients. Attributed to Dan Kurtz of MissPepper AI.](/library/ai-agent-seo/quote-morning-boot-claude.png "Adrian Nikolov, whose agency runs its client diagnostics the same way, on the first step of every engagement:

> That's a completely automated diagnostic that runs through all the MCPs. It crawls the website with screaming frog. pulls all of the relevant keyword data with data for Malte Landwehr, who runs product at Peec AI, on what that does to the tools themselves:
...
Read the data through one server, write through another, and the assistant is the thing in between.

## The setup, once

The [RANKMYSEO MCP](/docs/mcp) connects to Claude Code, Claude Desktop, Codex and Cursor. You add one server, sign in, and the assistant gains the tools: Search Console reads, keyword metrics, SERP results, domain and backlink data, rank tracking, site audit, local rank grids. Search Console and audit reads use no credits; research calls that hit a data provider do, and each tool says so before it runs.
...
![A terminal session in Claude Code calling the RANKMYSEO MCP get_search_console_performance tool for seoarcade.com over 28 days: the top rows are the brand name and real queries with clicks, impressions and position, and then a query that is the full text of an AI prompt beginning "you are a keyword analysis bot at an adtech company", with 31 impressions at position 10.2](/library/ai-agent-seo/mcp-search-console-query-pull-rankmyseo.png "Search Console through the RANKMYSEO MCP")
...
His episode is [on the industry Small Business podcast](https://industrysmallbusiness.com/mason-mccumber-on-ai-agents-agentic-workflows-building-smarter-business-systems-show-notes/). The data arrives faster than you could fetch it. The judgement about what it means did not get faster, and it is still yours. [What to automate and what to keep](/library/ai-agent-seo/what-to-automate) is about where that line goes.

## Run this with the RANKMYSEO MCP

Connect Search Console to the project and the [RANKMYSEO MCP](/docs/mcp) to your assistant. This prompt is the first of the five above, with the filter step the row above makes necessary.
...
### Which AI assistants work with the RANKMYSEO MCP?
