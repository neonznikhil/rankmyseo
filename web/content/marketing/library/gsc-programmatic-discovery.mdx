...
![A ranked-keyword list filtered to positions 10 through 13, showing the keyword, its position, and monthly search volume for each row](/library/keyword-research/gsc-striking-distance-rankmyseo.png)
...
Because the Search Console connection runs through RANKMYSEO, your agent can query it alongside your ranking data without a second integration. Connect the [RANKMYSEO MCP](/docs/mcp) once, or read how the [Search Console MCP](/google-search-console-mcp) fits together, then run the workflow with this prompt.

```text
Using the RANKMYSEO MCP, pull Search Console query data for
