...
const faqs = [
  ...
  {
    question: "What does the RANKMYSEO MCP give an agent?",
    answer:
...
export const Route = createFileRoute("/_marketing/library/ai-agent-seo/")({
  head: () =>
    buildPageSeo({
      ...
      description:
        "Four strategies for running SEO through an AI assistant: connect the MCP and run the first five prompts, decide what a schedule does and what stays with a person, keep the brief human, and make a good run repeatable with skills, memory and a trace. Each includes a workflow and an RANKMYSEO MCP prompt.",
      path: PATH,
    ...
    <article className="mx-auto max-w-5xl">
      <header className="max-w-3xl">
        ...
        <p className="mt-5 text-lg leading-8 text-[var(--color-brand-muted)]">
          ...
          happen again. Every one is built on real runs through the RANKMYSEO MCP
      ...
      <section className="mt-12">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-neutral-700">
          ...
          Every strategy is built on a real call through the RANKMYSEO MCP,
      ...
      <section className="mt-12">
        <h2 className="text-2xl font-semibold tracking-tight text-neutral-950">
          What the RANKMYSEO MCP gives an agent
        </h2>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--color-brand-muted)]">
      ...
      <section className="mt-12 flex flex-col items-start justify-between gap-4 rounded-xl border border-[var(--color-border-subtle)] bg-white p-6 sm:flex-row sm:items-center md:p-8">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-[var(--color-brand-muted)]">
            Each strategy ends with a copy-paste MCP prompt. RANKMYSEO is open
            source, free to start, and does not require a credit card.
          </p>
        </div>
        <a
          href="https://app.rankmyseo.com/sign-up"
