    ...
    <html lang="en" suppressHydrationWarning>
      <head>
        ...
        {/* Dub referral attribution: partner links land here with ?dub_id=
            ...
            the click id as a `dub_id` cookie on `.rankmyseo.com` so the app at
            app.rankmyseo.com can attribute the signup. Injected during the
            initial HTML parse (not idle-deferred like Plausible below) so the
            cookie lands before the visitor navigates to the app. */}
        <script
          dangerouslySetInnerHTML={{
            __html:
              "(function(){var q=new URLSearchParams(window.location.search);if(!q.has('dub_id')&&!q.has('via'))return;var s=document.createElement('script');s.src='https://www.dubcdn.com/analytics/script.js';s.dataset.domains='{\"refer\":\"links.rankmyseo.com\"}';s.dataset.cookieOptions='{\"domain\":\".rankmyseo.com\"}';document.head.appendChild(s)})();",
          }}
        />
        <script
          dangerouslySetInnerHTML={{
            __html:
              "(function(){function loadAnalytics(){if(window.__rankmyseoAnalyticsLoaded)return;window.__rankmyseoAnalyticsLoaded=true;window.plausible=window.plausible||function(){(plausible.q=plausible.q||[]).push(arguments)};plausible.init=plausible.init||function(i){plausible.o=i||{}};plausible.init({endpoint:'/api/event'});var script=document.createElement('script');script.defer=true;script.src='/js/script.js';document.head.appendChild(script)}function schedule(){if('requestIdleCallback'in window){window.requestIdleCallback(loadAnalytics,{timeout:2000});return}window.setTimeout(loadAnalytics,2000)}if(document.readyState==='complete'){schedule();return}window.addEventListener('load',schedule,{once:true})})();",
          }}
        />
      </head>
