    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const cacheKey = new Request(
          `https://rankmyseo.com/api/backlink-check/${domain}`,
        );
          ...
          try {
            ...
            if (used >= DAILY_CHECK_BUDGET) {
              return jsonResponse(
                {
                  error:
                    "The free checker has reached today's limit. Try again tomorrow, or sign up for RANKMYSEO for full backlink research.",
                },
                429,
              );
            }
