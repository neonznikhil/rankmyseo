...
RANKMYSEO preview stages use isolated Cloudflare resources and a shared,
Alchemy-managed Cloudflare Access boundary.

## Security model

- Preview Workers are named `rankmyseo-<stage>` and served from
  `rankmyseo-<stage>.<WORKERS_SUBDOMAIN>`.
- One persistent Access application protects
  `rankmyseo-*.<WORKERS_SUBDOMAIN>` before any preview Worker exists.
- Production uses the unsuffixed `rankmyseo` Worker on `app.rankmyseo.com` and
  `www.app.rankmyseo.com`. It does not match the preview wildcard and is not
...
```text
rankmyseo-*.your-subdomain.workers.dev
```
...
`https://rankmyseo-<stage>.<WORKERS_SUBDOMAIN>` — hosted previews use that as
...
External (rankmyseo/rankmyseo) PRs never deploy from CI — fork code must not
...
```sh
git fetch https://github.com/rankmyseo/rankmyseo.git pull/<pr>/head
git worktree add --detach ../rankmyseo-pub-<pr> FETCH_HEAD
cp .env.preview ../rankmyseo-pub-<pr>/
(cd ../rankmyseo-pub-<pr> && pnpm install --frozen-lockfile && pnpm exec vite build --mode preview)
rm -rf dist && cp -R ../rankmyseo-pub-<pr>/dist dist
git worktree remove --force ../rankmyseo-pub-<pr>
...
1. Append `--dry-run` to the alchemy command and read the plan — every prod
   resource (D1 `rankmyseo`, KV `every-super-seo`/`OAUTH_KV`, R2 `rankmyseo`,
   Hyperdrive `rankmyseo`, Worker `rankmyseo`) should be adopted, none created.
2. Diff `.env.production` against the live worker's secrets
   (`GET /accounts/:id/workers/scripts/rankmyseo/secrets`): alchemy's deploy
...
6. Note: prod already serves on `rankmyseo.<subdomain>.workers.dev` (alchemy
...
RANKMYSEO-specific and not required. The walkthrough lives in
