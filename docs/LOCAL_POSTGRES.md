# Running RANKMYSEO on Postgres locally

RANKMYSEO runs on **Cloudflare D1 (SQLite) by default**. Postgres is an opt-in
...
```sh
docker run --name rankmyseo-postgres \
  -e POSTGRES_USER=rankmyseo \
  -e POSTGRES_PASSWORD=rankmyseo \
  -e POSTGRES_DB=rankmyseo \
  -p 5433:5432 \
  -d postgres:16
```
...
```sh
docker exec rankmyseo-postgres pg_isready -U rankmyseo -d rankmyseo
```

The connection string is:

```
postgres://rankmyseo:rankmyseo@localhost:5433/rankmyseo
```
...
```sh
POSTGRES_DATABASE_URL=postgres://rankmyseo:rankmyseo@localhost:5433/rankmyseo \
  pnpm db:migrate:pg
```
...
docker exec rankmyseo-postgres psql -U rankmyseo -d rankmyseo -c "\dt"

# Inspect rows the app writes (e.g. after creating a project / saving keywords)
docker exec rankmyseo-postgres psql -U rankmyseo -d rankmyseo -c "select count(*) from projects;"
```
...
```sh
docker rm -f rankmyseo-postgres
```
