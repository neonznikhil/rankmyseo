...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database rankmyseo
   ```

2. If the output looks right, apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database rankmyseo --apply --confirm-remote-apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database rankmyseo --validate-only
   ```
...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database rankmyseo --local
   ```

2. Apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database rankmyseo --local --apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database rankmyseo --local --validate-only
   ```
