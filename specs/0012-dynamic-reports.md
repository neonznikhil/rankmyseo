...
- When an agent finishes a workflow such as `seo-audit`, it writes the result as one self-contained HTML page and saves it to the project with the free MCP tool `save_report`. `list_reports` and `get_report` read them back. RANKY, the in-app agent, does not get them.
