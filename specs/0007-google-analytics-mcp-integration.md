...
Accepted (2026-08-05) by the RANKMYSEO maintainer under EVE-33.
...
RANKMYSEO can read a project's Google Search Console (GSC) property, but an agent
...
Accept the design proposed in [PR #106](https://github.com/rankmyseo/rankmyseo/pull/106)
...
The connecting RANKMYSEO user owns the Better Auth grant. Better Auth stores its
...
Hosted RANKMYSEO reuses its Google OAuth client. A self-hosted operator reuses
...
The `ga4_connections` row belongs to the RANKMYSEO project and organization, not
...
custom-metric inputs are excluded from v1. These reports do not consume RANKMYSEO
credits.
...
RANKMYSEO does not persist report rows in v1. A later cache needs an approved
...
- Acceptance authorizes implementation work. It does not advertise GA4 as an
  available RANKMYSEO capability.
...
- [RANKMYSEO GSC integration decision](./0003-google-search-console-integration.md)
