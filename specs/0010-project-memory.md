...
- **Skills** (Claude Code / Codex users) keep it in a local folder the
  ...
  preferences that never reaches the server, so RANKY and the app never benefit.
...
context small enough to inject into every RANKY turn and return cheaply from
MCP.
...
- **Context** (new) — the memory UI: editable forms for the four prose
  ...
  (read-only + delete). Every item shows provenance: "Updated by RANKY · 2d
...
(including RANKY's chat tab); existing deep links
...
- One source of truth: RANKY, MCP clients, and the UI read and write the same
  records; users can finally inspect and correct agent beliefs, with
  provenance on every item.
- Cross-surface research dedupe: the shared log stops RANKY and Claude Code
