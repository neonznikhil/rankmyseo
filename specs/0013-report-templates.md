...
- Two free MCP tools, `list_report_templates` and `save_report_template` (create or update); RANKY, the in-app agent, does not get them. `save_report` accepts an optional template id, and the reports list shows the template name as the report's type.
